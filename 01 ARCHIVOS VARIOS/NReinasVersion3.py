...
class Maestro:
    ...
    def nReinasLasVegas(self, n):
        ...
        listaCols.pop(listaCols.index(columnaInicial))
        t = 1
        for i in range(n-1):
            aleatorio = random.sample(listaCols, 1)[0]
            t = t + 1
            if self.validar(aleatorio, listaResultado, n):
                listaResultado.append(aleatorio)
                listaCols.pop(listaCols.index(aleatorio))
            else:
...
class Simulador():
    ...
    def empezar(self, retadores, n, tiempo, desviacion):
        t_entre_llegada = [0]
        t_llegada = [0]
    ...
    def analisisDeCola(self,n,retadores,desviacion,minCola,numMuestras,numRepeticiones):
      resultado=self.m.analisisAlgoritmoProb(numMuestras,numRepeticiones)
      promVegas=int(resultado[0])
      promDeter = resultado[1]
        
      for x in range(promDeter,promVegas+1):
        cola = self.empezar(retadores,n,x,desviacion)[1]
        if cola<=minCola:
          return "El tiempo adecuado de llegadas para una cola minima de "+str(minCola)+ "es : "+str(x) 
      return "pailas"  
...
if __name__ == "__main__":
    ...
    '''
    m=Maestro()
    x = m.analisisAlgoritmoProb(1,1)
    print(str(x))
    
    print(simulacion.analisisDeCola(8,100,0,10,100,1))
